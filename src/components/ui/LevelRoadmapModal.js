...
export async function showLevelRoadmapModal() {
  ...
  const modalHtml = `
    <div class="lrm-overlay" id="level-roadmap-overlay">
      <div class="lrm-popup">
        <!-- Header -->
        <div class="lrm-header">
          <button class="lrm-close-btn" aria-label="Đóng" id="btn-close-level-roadmap">
          ...
          <div class="lrm-progress-bar">
            <div class="lrm-progress-fill" style="width: ${progressPercent}%;"></div>
          </div>
        </div>

        <!-- Timeline -->
        <div class="lrm-timeline">
          ${allLevels.map((lvl) => {
            const isCurrent = currentLevel.level === lvl.level;
            const isPassed = currentLevel.level > lvl.level || totalXp >= lvl.minXp;

            return `
              <div class="lrm-item ${isCurrent ? 'current' : ''}">
                <div class="lrm-dot" style="${isPassed ? 'background: linear-gradient(135deg, #8b5cf6, #7c3aed);' : 'background: #e5e7eb; box-shadow: none;'}">
                  ${isPassed ? `
                ...
                <div class="lrm-card">
                  <div class="lrm-name">Level ${lvl.level} - ${lvl.name}</div>
                  <div class="lrm-reward">
                  ...
                  <div class="lrm-xp">Mốc: ${lvl.minXp} XP</div>
                </div>
              </div>
            `;
          }).join('')}
